...
- **맞춤형 궁합 분석**: 두 사람의 생년월일과 시간을 기반으로 음양오행 상생/상극 관계를 분석하여 정확한 궁합 정보 제공
...
1. **궁합 보기 초기 화면**
   - 두 사람의 정보 입력: 이름, 성별, 생년월일, 태어난 시간
   - 직관적인 입력 폼과 가이드 텍스트 제공
   - 입력 완료 후 [궁합 점쳐보기] 버튼 활성화
2. **로딩 화면**
   - 3단계 애니메이션으로 궁합 분석 과정 시각화
   - 각 단계별 설명 메시지로 사용자 몰입 유도
   - 고양이 캐릭터가 재미있는 멘트로 사용자 경험 향상
3. **결과 화면**
   - 궁합 점수 및 간략한 요약 정보 상단 노출
   - 음양오행 분석: 두 사람의 오행 관계 시각화 및 설명 제공
   - 카테고리별 분석: 성격 궁합, 연애 스타일, 갈등 요소, 미래 전망 등
   - 하단에 공유 버튼으로 SNS 및 링크 공유 기능 제공
...
| 사주 궁합 분석            | 두 사람의 정보를 바탕으로 음양오행 궁합 분석 제공     | 어려움      | 상       |
| 궁합 결과 공유            | 분석된 궁합 결과를 SNS 및 링크로 공유하는 기능        | 쉬움        | 중       |
| 궁합 분석 로딩 애니메이션 | 궁합 분석 중 3단계 로딩 애니메이션으로 몰입감 제공    | 중간        | 중       |
