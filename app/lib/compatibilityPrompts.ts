...
중요: 반드시 JSON 형식만 응답하세요. 설명이나 추가 텍스트 없이 JSON 객체만 반환해야 합니다.
...
중요: 응답은 반드시 JSON 객체만 포함해야 합니다. 추가 설명이나 마크다운 코드 블록 등을 사용하지 마세요. 
오직 { } 중괄호로 시작하고 끝나는 순수한 JSON 객체만 반환하세요.`;

// 친구 궁합 분석 프롬프트
export const friendCompatibilityPrompt =
  `당신은 사주와 운세에 특화된 AI 운세 상담사입니다.
...
중요: 응답은 반드시 JSON 객체만 포함해야 합니다. 추가 설명이나 마크다운 코드 블록 등을 사용하지 마세요.` +
  `예를 들어 "\`\`\`json" 같은 코드 블록 마커를 사용하지 마세요.
오직 { } 중괄호로 시작하고 끝나는 순수한 JSON 객체만 반환하세요.`;
