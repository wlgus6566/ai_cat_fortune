...
const talismanTemplates = {
    // 기본 템플릿
    default: (concern: string) => `A modern pastel illustration of a Japanese-style lucky charm (omamori) with a cute, chubby cat character, expressing encouragement related to: "${concern}". The charm includes Japanese aesthetic elements, decorative knots, and spiritual symbols. The talisman displays relevant encouraging words in English. Style: Kawaii, pastel colors, soft edges, warm and magical feeling. Aspect ratio 9:16.`,
    
    // 연애 관련 템플릿
    romance: (concern: string) => `A beautifully designed Japanese love charm (omamori) with heart motifs and soft pink/red colors. Features a cute cat character and romantic symbols. The talisman includes an encouraging message about love: "${concern}". Style: Romantic, dreamy, kawaii, with soft pastel colors. Aspect ratio 9:16.`,
    
    // 직장/커리어 관련 템플릿
    career: (concern: string) => `A professional-looking Japanese success charm (omamori) with elements symbolizing career growth and achievement. Features a determined-looking cat character in business attire. Includes an encouraging message about career: "${concern}". Style: Professional with playful elements, blue and gold color scheme. Aspect ratio 9:16.`,
    
    // 금전 관련 템플릿
    money: (concern: string) => `A prosperity-focused Japanese wealth charm (omamori) with gold coins, money symbols and lucky elements. Features a happy cat character with a coin. Includes an encouraging message about finances: "${concern}". Style: Abundant, prosperous, with green and gold colors. Aspect ratio 9:16.`,
    
    // 심리 관련 템플릿
    psychology: (concern: string) => `A calming Japanese healing charm (omamori) with elements symbolizing mental peace and balance. Features a meditating cat character with serene expression. Includes a comforting message about mental well-being: "${concern}". Style: Serene, soothing, with purples and blues. Aspect ratio 9:16.`,
    
    // 인간관계 관련 템플릿
    relationships: (concern: string) => `A harmonious Japanese friendship charm (omamori) with elements symbolizing connection and social bonds. Features multiple cute cat characters together. Includes an encouraging message about relationships: "${concern}". Style: Warm, connected, with yellows and oranges. Aspect ratio 9:16.`,
    
    // 라이프스타일 관련 템플릿
    lifestyle: (concern: string) => `A balanced Japanese lifestyle charm (omamori) with elements symbolizing harmony and life enjoyment. Features a content cat character engaged in various activities. Includes an encouraging message about life balance: "${concern}". Style: Harmonious, diverse, with balanced color palette. Aspect ratio 9:16.`
};
...
function getRandomPhrase(category: string): string {
    ...
    const mappedCategory = 
        category.toLowerCase().includes('romance') || category.toLowerCase().includes('연애') ? 'romance' :
        category.toLowerCase().includes('career') || category.toLowerCase().includes('직장') || category.toLowerCase().includes('커리어') ? 'career' :
        category.toLowerCase().includes('money') || category.toLowerCase().includes('금전') || category.toLowerCase().includes('돈') ? 'money' :
        category.toLowerCase().includes('psychology') || category.toLowerCase().includes('심리') ? 'psychology' :
        category.toLowerCase().includes('relationships') || category.toLowerCase().includes('인간관계') ? 'relationships' :
        category.toLowerCase().includes('lifestyle') || category.toLowerCase().includes('라이프') || category.toLowerCase().includes('생활') ? 'lifestyle' : 'default';
    
    // 해당 카테고리의 문구 배열
    const phrases = encouragingPhrases[mappedCategory as keyof typeof encouragingPhrases] || 
...
function getTemplateByCategory(category: string, concern: string): string {
    ...
    // 템플릿 함수 가져오기
    const templateFn = talismanTemplates[mappedCategory as keyof typeof talismanTemplates] || 
    ...
    try {
            ...
            return NextResponse.json(
                {
                    ...
                    error: {
                        code: 'INVALID_PROMPT',
                        message:
                            'The prompt is missing or exceeds length limitations.'
                    }
                } as IErrorResponse,
                { status: 400 }
            )
        ...
        const prediction = await replicate.predictions.create({
            ...
            input: {
                ...
                output_format: 'webp',
                output_quality: 90,
                negative_prompt: 'blurry, low quality, distorted, deformed, text in image that is unreadable or messy, misaligned features' // 품질 향상을 위한 네거티브 프롬프트 추가
            }
        })
