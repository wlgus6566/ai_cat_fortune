...
interface UserContextType {
  ...
  createUserProfile: (data: Omit<UserProfile, 'id' | 'createdAt' | 'updatedAt'>) => Promise<void>;
...
export function UserProvider({ children }: { children: ReactNode }) {
      ...
      try {
        if (session?.user) {
          ...
          try {
            ...
            if (error) {
              // 테이블이 없거나 다른 오류인 경우에도 기본 프로필로 진행
              console.warn('사용자 프로필을 로드하는 중 오류가 발생했습니다:', error);
              // 기본 프로필 생성
            ...
            if (data) {
              setUserProfile({
                ...
                profileImageUrl: data.profile_image_url || session.user.image || '',
          ...
          if (typeof window !== 'undefined') {
            try {
              ...
              console.error('로컬 스토리지에서 프로필을 로드하는 중 오류가 발생했습니다:', error);
              setUserProfile(null);
            }
          }
        }
  ...
  useEffect(() => {
    if (isLoaded && userProfile && !isAuthenticated && typeof window !== 'undefined') {
      localStorage.setItem(USER_PROFILE_KEY, JSON.stringify(userProfile));
    }
  }, [userProfile, isLoaded, isAuthenticated]);

  // 사용자 프로필 생성
  const createUserProfile = async (data: Omit<UserProfile, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
      const now = new Date().toISOString();
      ...
      if (session?.user?.id) {
        ...
        try {
          ...
          if (checkError && checkError.code === '42P01') {
            console.warn('user_profiles 테이블이 존재하지 않습니다. 로컬 스토리지에만 저장합니다.');
            ...
            if (error) {
              console.warn('Supabase에 프로필 저장 실패, 로컬 스토리지에만 저장합니다:', error);
              // 로컬 스토리지에 저장
              if (typeof window !== 'undefined') {
                localStorage.setItem(USER_PROFILE_KEY, JSON.stringify(newProfile));
              }
          ...
          console.warn('Supabase 연결 실패, 로컬 스토리지에만 저장합니다:', error);
          // 로컬 스토리지에 저장
          if (typeof window !== 'undefined') {
            localStorage.setItem(USER_PROFILE_KEY, JSON.stringify(newProfile));
          }
      ...
      if (session?.user?.id) {
        ...
        try {
          ...
          if (checkError && checkError.code === '42P01') {
            ...
            if (error) {
              console.warn('Supabase에 프로필 업데이트 실패, 로컬 스토리지에만 저장합니다:', error);
              // 로컬 스토리지에 저장
              if (typeof window !== 'undefined') {
                localStorage.setItem(USER_PROFILE_KEY, JSON.stringify(updatedProfile));
              }
