...
// 공개 경로 정의
const publicPaths = ["/auth/signin", "/auth/error", "/api/auth", "/setup"];

// 정적 파일 및 API 경로 체크 헬퍼 함수
const isStaticFile = (pathname: string) => {
  return /\.(js|css|png|jpg|jpeg|gif|ico|svg|webp|woff|woff2|ttf|eot)$/i.test(
    pathname
  );
};
...
export async function middleware(request: NextRequest) {
  ...
  // 인증 세션 체크 - getToken을 사용하여 JWT 토큰 확인
  const token = await getToken({
    req: request,
    secret: process.env.NEXTAUTH_SECRET,
  });

  console.log("미들웨어 인증 확인:", { pathname, isAuthenticated: !!token });
