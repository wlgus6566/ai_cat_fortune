  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-800'}`}>
      ...
      <PageHeader 
        ...
        className={`${darkMode ? 'bg-gray-900 text-white border-gray-800' : 'bg-white text-gray-800'}`}
      />
        ...
        >
          ...
          >
              <div className="flex items-start">
                <span className={`text-xl mr-3 ${darkMode ? 'text-purple-400' : 'text-purple-600'}`}>✨</span>
                <div>
                  {/* <h3 className={`text-lg font-semibold ${darkMode ? 'text-white' : 'text-gray-800'} mb-2 text-left`}>{t('talisman.collectionTitle')}</h3> */}
                  <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'} text-left`}>
          ...
          {/* 본문 섹션 */}
          <motion.section
            className={`${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-lg shadow-md overflow-hidden border mb-6`}
            variants={itemVariants}
          >
            {isLoading ? (
              ...
              <div className={`${darkMode ? 'bg-red-900 border-red-800' : 'bg-red-50 border-red-200'} border rounded-md p-4 m-4`}>
                <p className={`${darkMode ? 'text-red-300' : 'text-red-500'}`}>{error}</p>
              </div>
            ) : talismans.length === 0 ? (
              <motion.div 
                className={`${darkMode ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'} border rounded-md p-6 m-4 text-left`}
                variants={itemVariants}
              >
                <div className="flex items-start">
                  <span className={`text-2xl mr-3 ${darkMode ? 'text-yellow-400' : 'text-yellow-500'}`}>🔮</span>
                  <div>
                    <p className={`${darkMode ? 'text-gray-300' : 'text-gray-500'} font-medium mb-2`}>{t('talisman.emptyState')}</p>
                    <motion.p 
              ...
              >
                ...
                >
                  {talismans.map((talismanUrl, index) => (
                    <motion.div 
                      key={index} 
                      className={`border ${darkMode ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-white'} rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow cursor-pointer`}
                    ...
                    >
                      ...
                      <div className={`p-2 text-left ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                        <p className="text-sm font-medium">{t('talisman.talismanNumber', { number: index + 1 })}</p>
                      </div>
                    </motion.div>
                  ))}
                </motion.div>
          ...
          {/* 푸터 */}
          <motion.footer 
            className={`text-left my-6 ${darkMode ? 'text-gray-400' : 'text-gray-500'} text-sm flex items-center`}
            variants={itemVariants}
          >
